...
The legacy draft explicitly selected:
```text
...
```
No routing provider is currently selected; route optimization remains deferred for Deliver Order.
...
Routing Matrix and Optimization
```
is not selected by the current contract; route optimization remains deferred.
## 25. Route Authority
an approved routing service may provide:
```text
distance
travel time
...
Open Decision.
## 55. ETA
an approved routing service may support estimated travel time.
...
Conceptual:
```http
GET /api/v1/courier/delivery-tasks/{taskId}/delivery
```
## 90. Current Active Task
Possible:
```http
GET /api/v1/courier/delivery-tasks/active
```
## 91. Route Context
Routing may be resolved server-side through a routing provider adapter or returned as route-ready data.
Exact API is Open.
## 92. Location Update
If live GPS is enabled:
```http
POST /api/v1/courier/delivery-tasks/{taskId}/location
```
...
## 105. Map Credentials
routing provider credentials/tokens must be configured safely according to the selected mobile/server integration.
...
```
## 117. Routing Provider Boundary Failure
```text
...
## 122. Route Requests
Avoid repeated unnecessary routing provider route calculations.
...
## 135. Routing Provider Boundary
The legacy draft selected a provider, but the current contract does not:
```text
Routing Matrix and Optimization
```
for Rider/Logistics optimal routing.
This is the relevant source-selected external API for Deliver Order.
## 136. Routing Provider Boundary
`Courier.md` gives routing provider as an example mapping API.
It is not an additional mandatory provider because the current contract does not select a routing provider; this remains deferred.
...
- routing integration consistent with a future approved provider contract
...
- an approved routing service may provide route/distance/travel context.
- routing provider failure does not fabricate route or ETA.
...
- server-side routing provider secrets are not exposed.
- payment/security data is absent.
## 155. Third-Party
- No routing provider is currently selected; route optimization remains deferred.
- A routing provider is only a future option, not an additional mandatory dependency.
...
- GPS unavailable
- routing provider route failure
- network failure
- reconnect
- Chat navigation
- Incident navigation
