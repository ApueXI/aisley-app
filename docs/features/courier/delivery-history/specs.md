...
- Current filtering accepts exact Order reference only; from/to UTC timestamps and cursor semantics in this section are deferred targets.
...
- A 404 detail response shows unavailable without revealing another Courier's task; the current controller does not guarantee a custom `DELIVERY_HISTORY_NOT_FOUND` code.
