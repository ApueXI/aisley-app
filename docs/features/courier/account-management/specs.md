...
## 7. Shared User Model Context
the shared backend contract says:
...
Conceptual:
```http
PATCH /api/v1/courier/account/profile
```
## 78. Vehicle Update
Conceptual:
```http
PATCH /api/v1/courier/account/vehicle
```
or:
```http
POST /api/v1/courier/account/vehicle-change-request
```
...
Conceptual:
```http
PATCH /api/v1/courier/account/license
```
or a pending-change endpoint where verification is required.
## 80. Payout Method
Conceptual:
```http
GET /api/v1/courier/account/payout-method
PATCH /api/v1/courier/account/payout-method
```
Exact implementation depends on payout architecture.
## 81. Password Change
Conceptual:
```http
POST /api/v1/courier/account/password
```
## 82. Email Update
Only if supported:
```http
PATCH /api/v1/courier/account/email
```
## 83. Pending Sensitive Changes
Possible:
```http
GET /api/v1/courier/account/change-requests
```
...
routing provider, SMS, and Push are not required for core Account Management.
...
- No payout provider is required merely to store payout settings.
- routing provider/SMS/Push are not required.
# Tests
## 192. Backend Tests
Test:
