...
From the shared backend contract, Flutter uses:
```text
...
Deliver Order
```
## 49. Routing Provider Boundary
The current contract does not select a routing provider. Route optimization remains deferred to the Logistics/routing contract, and the Dashboard itself must not calculate routes independently.
...
```http
GET /api/v1/courier/delivery-requests
```
## 85. Active Tasks
Alternative:
```http
...
Possible:
```http
GET /api/v1/courier/notifications
```
...
The legacy draft named a routing provider for:
```text
optimal route system
for Logistics and Riders
```
It is relevant when the Courier opens route/navigation behavior.
The basic Dashboard queue does not need independent route calculation.
## 123. Brevo
...
- A routing provider is not required merely to list requests.
