...
```
## 32. Route Preview
The request review may consume route/distance data generated from a routing provider.
## 33. Routing Provider Boundary
A routing service provides:
```text
...
Conceptual:
```http
GET /api/v1/courier/delivery-requests/{taskId}
```
## 55. Accept
Conceptual:
```http
POST /api/v1/courier/delivery-requests/{taskId}/accept
```
...
```http
POST /api/v1/courier/delivery-requests/{taskId}/decline
```
...
## 82. Route Preview
If a routing provider is used, route requests should be bounded and cached where appropriate.
...
```
## 96. Routing Provider Boundary
No routing provider is currently approved for route/distance preview.
## 97. Brevo
Not required.
## 98. SMS / Push
...
## 100. Recommended
- confirmation step
- route preview via routing provider
...
- an approved routing service may provide route context.
...
13. routing provider failure behavior
