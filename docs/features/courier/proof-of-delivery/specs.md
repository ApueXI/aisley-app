...
(e.g., configured private blob storage)
...
configured private blob storage is not a mandatory vendor selection.
AISLEY may use:
```text
configured private blob storage
configured private blob storage
...
Conceptual:
```http
GET /api/v1/courier/delivery-tasks/{taskId}/proof-of-delivery
```
## 64. Submit Photo
Possible:
```http
POST /api/v1/courier/delivery-tasks/{taskId}/proof-of-delivery/photo
```
## 65. Submit Signature
Possible:
```http
POST /api/v1/courier/delivery-tasks/{taskId}/proof-of-delivery/signature
```
## 66. Verify QR
Possible:
```http
POST /api/v1/courier/delivery-tasks/{taskId}/proof-of-delivery/qr
```
## 67. Unified Endpoint
A unified endpoint is also valid:
```http
POST /api/v1/courier/delivery-tasks/{taskId}/proof-of-delivery
```
Exact API shape is Open.
## 68. Evidence Detail
Conceptual:
```http
GET /api/v1/courier/delivery-tasks/{taskId}/proof-of-delivery/{proofId}
```
...
## 102. configured private blob storage
configured private blob storage is a source example, not a required vendor.
## 103. Camera / QR
Flutter may use local camera/QR libraries.
## 104. Signature
Flutter may use a local signature-capture library.
## 105. Routing Provider Boundary
...
## 110. Not Required
- configured private blob storage specifically
...
- configured private blob storage is not hard-required unless selected.
