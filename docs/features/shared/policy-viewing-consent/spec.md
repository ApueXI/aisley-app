...
canonical: true
source_coverage: docs/requirements.md, docs/workspace.md, docs/schema.md, docs/domain/*, Admin Manage Platform Settings, role auth specs
---
...
- Current status response is `{data: {policies: [...], all_required_accepted: boolean}}`; `current_version` and `accepted_version` are nullable objects containing integer `version`, not bare integers. A direct DTO/numeric-string parser is client compatibility, not the canonical envelope.
- With enforcement disabled, `required` is false and `all_required_accepted` is true even when `accepted` is false. Use those server gate fields; never fabricate acceptance or maintain an independent mandatory gate. Recheck on protected `403 POLICY_CONSENT_REQUIRED` after re-enabling.
- No Courier endpoint exposes an editable enforcement switch. Admin controls and their UI remain upstream-only.
...
- Project contracts: `docs/requirements.md`, `docs/workspace.md`, `docs/schema.md`, `docs/domain/Admin.md`, `docs/domain/Buyer.md`, `docs/features/admin/manage-platform-settings/spec.md`, and the role auth specifications.
