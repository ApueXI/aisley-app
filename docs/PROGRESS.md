...
- **Copied backend documentation baseline:** local Laravel checkout `94e3467` (2026-09-24). The Flutter inbox was adopted earlier against `feature/courier-notifications` / `courier-notifications-v1`; historical entries retain the commits and actual tests for each client feature. Updating copied backend documents does not upgrade Flutter code.
...
- **Still deferred or unverified:** Dashboard operational aggregation, physical camera acceptance on installed targets, background push/WebSockets, live location telemetry, signature proof, Flutter chat UI, earnings, offline synchronization, browser CORS/private upload reads, and installed-device upload acceptance. Laravel now exposes task-scoped Courier chat with Logistics, Seller, and Buyer; the copied chat contract is not evidence of client adoption.
...
- Refreshed this documentation bundle against Laravel checkout `94e3467`: copied current shared/domain/operational contracts, the Courier task-chat spec and API handoff, and the Logistics chat counterpart spec. Updated the Flutter-owned index, architecture, and design to distinguish implemented chat APIs from absent Flutter screens, partial photo-POD/intent adoption from verified delivery, and the new COD declaration requirement. No Flutter source, Laravel code, database, live cross-role exchange, or installed-device behavior was changed or tested by this documentation sync.
