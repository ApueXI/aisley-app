...
- **Flutter implementation recorded here:** Auth/account/vehicle/policy, Courier notification list/detail/unread-count/mark-read with unread badge, legacy first-/final-mile task screens, QR/Code 128 candidate scanning, older QR/reference delivery-proof/completion UI, and delivered-history screens. Linux remains manual-input only. The latest recorded Flutter notification run passed 129 tests; a later multipart-transport run passed 134 tests with analyzer, web, and APK builds.
- **Current Laravel contract not yet verified in this Flutter client:** task-bound final-mile hub handoff without an identifier; 1–15 parcel batch acceptance and advisory route/map; private photo POD and photo-linked Delivered intent; failed-attempt/rejection/retry flow; and assigned company-truck Linehaul trip reads. The old QR/reference delivery-proof UI is incompatible with the current photo-only endpoint. `courier-linehaul.trip-scheduled` is an informational inbox alert, but navigation to a trip screen is not established.
