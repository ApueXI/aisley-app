...
The Laravel API supports Courier authentication, account/profile photo, vehicle management, shared policy consent, support tickets, the notification inbox, both task legs, first-mile pickup, task-bound final-mile hub handoff, private photo POD/Logistics-reviewed completion, failed attempts, advisory batch routes, delivered history, assigned company-truck trip reads, and task-scoped text chat with Logistics, Seller, and Buyer. Flutter implements photo selection/upload and completion intent with COD cash confirmation, the notification inbox, separate read-only dashboard task previews, and task-chat inbox with Logistics/Seller sending. Support tickets, normal batch acceptance, batch-route rendering, failed-attempt submission, and linehaul trip screens remain unadopted. Buyer chat remains read-only. Authenticated COD/Logistics validation, live chat exchange, and installed-device/browser acceptance remain unverified. The dashboard aggregate remains a scaffold. Use `docs/PROGRESS.md` for dated client evidence and `docs/features/courier/README.md` for API availability and draft boundaries.
...
The copied backend contracts were fully reconciled against local Laravel checkout `ca1487c` on 2026-09-25. Feature-level `backend_contract_commit` values may identify the implementation commit for an older slice. Flutter-owned architecture, design, rules, and progress preserve client-adoption history; copying a contract does not implement its UI. Current COD completion requires server-validated `cod_collected: true`: the authorized delivery read provides `data.order.payment_method`, `data.order.payable_total`, and `data.order.currency`; `data.parcel.price` is merchandise subtotal. Final-mile batch acceptance uses an empty JSON body, no idempotency header, and a state-idempotent current projection. Courier support-ticket routes exist, but Flutter screens remain pending. Recheck the live API and record the adopted backend commit/API version when changing Flutter code. Update Flutter `docs/PROGRESS.md` after implementation, test, contract, or material documentation changes; retain historical entries.
