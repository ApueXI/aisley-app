...
- Implemented the Courier task-chat client against copied Laravel checkout `94e3467` / API `courier-operational-messaging-v2`: protected inbox, task-scoped Logistics conversations, plain-text history, read markers, bounded pagination, visible-screen polling, and dashboard/pickup/delivery entry points. Seller and Buyer threads are read-only in this rollout because their counterpart reply screens have not been verified.
- Added bearer-auth repository parsing and controller/UI handling for scoped denial, policy consent, stale/offline/timeout states, throttling, idempotent exact-request retry, server-confirmed messages, and private in-memory cleanup on session changes. No Laravel code, new package, or local chat persistence was added.
- Verification: unauthenticated live Courier inbox returned `401`, confirming route protection only; authenticated response, cross-role exchange, reassignment/terminal behavior, and installed-device/browser runtime acceptance remain unverified. `flutter analyze`, full `flutter test` (167 passed), `flutter build web`, `flutter build apk --release`, and `git diff --check` passed. The chat spec remains 230 physical lines.
